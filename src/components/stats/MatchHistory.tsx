...
export default function MatchHistory({ matches, profiles, onDeleteMatch, onDeletePlayerMatches, onResetAll }: Props) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (confirmAction.type === 'match' && confirmAction.id) {
        await onDeleteMatch(confirmAction.id);
            ...
            <div key={m.id} className="flex items-center gap-2 rounded-xl bg-card p-3 shadow-sm">
              ...
              <button
                onClick={() => setConfirmAction({
                  type: 'match',
                  id: m.id,
                  label: `Delete match ${m.player1_name} ${m.player1_score}-${m.player2_score} ${m.player2_name}? Stats for both players will be recalculated.`,
                })}
