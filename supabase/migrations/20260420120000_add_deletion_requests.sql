...
-- Re-create core delete helpers (idempotent) to ensure they exist on the live DB.
-- The previous migration (20260413) may not have been applied yet.
...
-- ── Deletion requests ────────────────────────────────────────────────────────
-- Tracks per-match deletion consent between two players managed by different
-- authenticated users. When both profiles belong to the same auth user the
-- request_match_deletion RPC deletes directly and returns NULL.
...
-- Request deletion of a finished match.
-- Returns NULL  → both profiles share the same auth user; match was deleted directly.
-- Returns UUID  → a deletion_requests row was created; other player must approve.
CREATE OR REPLACE FUNCTION public.request_match_deletion(
  p_match_id              uuid,
  p_requesting_profile_id uuid
)
...
BEGIN
  ...
  -- Same owner → delete immediately, no consent needed
  ...
  -- Cancel any existing pending request for this match by the same requester
...
-- Accept or decline a pending deletion request.
-- Returns TRUE  → request accepted, match deleted, stats recalculated.
-- Returns FALSE → request declined (or not found).
...
-- Enable realtime so clients receive live updates on deletion_requests changes
